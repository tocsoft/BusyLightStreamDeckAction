...
namespace BusyLightStreamDeckAction
{
    public class OpenhabManager
    {
        private static List<OpenhabManager> managers = new List<OpenhabManager>();
        public static OpenhabManager Server(string url)
        {
            ...
            if (manager == null)
            {
                lock (managers)
                {
                    ...
                    if (manager == null)
                    {
                        manager = new OpenhabManager(url);
        ...
        public OpenhabManager(string url)
        {
            Url = url;
        }
        ...
        {
            public MonitorDisposable(OpenhabManager manager, string itemName, Action<string> callback)
            {
                Manager = manager;
                ItemName = itemName;
                Callback = callback;
            ...
            private OpenhabManager Manager { get; set; }
